...
sudo mv ./bin/sphnctl /usr/local/bin
```
...
```sh
sphnctl keys create --name [Wallet Name]
```
...
```sh
sphnctl keys use --name [Wallet Name] --key-secret [Key Secret]
```
...
<Navigation name="Spheron Protocol CLI" />
