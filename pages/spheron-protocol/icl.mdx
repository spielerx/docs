...
1. [Network Configuration](/spheron-protocol/icl#1-network-configuration) 
2. [Version](/spheron-protocol/icl#2-version-configuration)
3. [Services](/spheron-protocol/icl#3-services-configuration)
4. [Profiles](/spheron-protocol/icl#4-profiles)
5. [Deployment](/spheron-protocol/icl#5-deployment-configuration)
6. [GPU Support](/spheron-protocol/icl#6-gpu-integration-support-in-compute-profiles)
...
| depends-on | No                 | Reserved for future functionality. Currently has no effect on deployments.                                                                    |
...
| env        | No                 | Sets environment variables for the running container. Refer to the [services.env](/spheron-protocol/icl#3-services-configuration) section for more details.                                     |
| expose     | No                 | Determines which entities are permitted to connect to the services. See the [services.expose](/spheron-protocol/icl#port-exposure-in-servicesexpose) section for additional information.               |

### Environment Variables for `services.env`
...
```
### Port Exposure in `services.expose`
...
-   **Signed Certificates:** To implement properly signed certificates, you'll need to use a third-party solution like Cloudflare as a front-end. We provide documentation on integrating Cloudflare with Spheron if you're interested in this approach.
...
| `to`           | No                 | Enumerates the entities permitted to connect to this port. Refer to the `services.expose.to` section for more details.  |
...
### Configuring `services.expose.to`
...
## 4. Profiles
The profiles section is used to define named compute and placement profiles that can be referenced in the [deployment section](/spheron-protocol/icl#5-deployment-configuration).
...
This defines a profile named `web` with resource requirements of 4 vCPUs, 4 gigabytes of memory, and 10 gigabytes of storage.
...
This defines a profile named `eastcoast` with the required attributes `{region="us-east"}` and a maximum price of 8000000 (8 USDT with a precision of 6) per hour for the web compute profiles.

Providers can only assign themselves to a predefined set of regions. If you specify an unlisted region, no provider will bid on your deployment, causing it to fail automatically. Please refer to the list mentioned in [this section].
...
web:
  eastcoast:
    ...
    count: 30
```
This specifies that 30 instances of the `web` service should be deployed to a datacenter that matches the `eastcoast` datacenter profile. 
Each instance will have access to the resources defined in the `web` compute profile.
...
For a comprehensive example of a GPU-enabled ICL, refer to this [example](https://github.com/spheronFdn/docs/tree/main/examples/protocol) which includes the declaration of several GPU models.
...
<Navigation name="Infrastructure Composition Language (ICL)" />
