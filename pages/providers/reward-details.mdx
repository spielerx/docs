...
- **Uptime Calculation**: The Slark node calculates uptime by counting the number of successful challenges out of the total challenges within an Era.
...
- **Promotion and Demotion**: The Slark node also determines if a provider should be promoted or demoted based on their performance over multiple Eras. Consistent good performance can lead to promotion to a higher tier, while consistent poor performance can lead to demotion to a lower tier.
