...
```
sphnctl provider add --from ~/.spheron/wallet.json --key-secret testPassword --config /home/spheron/.spheron/provider-config.json
```

<Callout type="info">
Note: You can modify and change provider details using below command:
```
provider update --from ~/.spheron/wallet.json --key-secret testPassword --config /home/spheron/.spheron/provider-config.json
```
</Callout>
