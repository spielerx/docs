...
4. Provide the domain you attached in the [playbook hostname](/providers/prepare-linux-instances#step-3-edit-playbook).
...
## Deposit SPH to the provider's wallet
...
- **GPU:** List all the GPU models supported by your provider. Use the shorthand values from the [GPU support](/user-guide/supports#gpu-support--tiering) page. Ensure you include the pricing for each GPU model in the Pricing section.
- **CPU:** List all the types of CPUs you support. Options include:
  - **basic:** Older generation server hardware
  - **gp:** General purpose server hardware
  - **hpc:** High-performance compute server hardware
- **RAM:** List all types of RAM you support. Options include: ddr2, ddr3, ddr4.
...
Once you have updated the configuration values, create a `config.json` file for your provider at `/home/spheron/.spheron/config.json`. To do this:
...
```
vi /home/spheron/.spheron/config.json
```

3. Paste the updated configuration into the config.json file and save it.
...
```
sphnctl provider add --from ~/.spheron/wallet.json --key-secret testPassword --config config.json 
```

<Callout type="info">
Note: You can modify and change provider details using below command:
```
provider update --from ~/.spheron/wallet.json --key-secret testPassword --config config.json
```
</Callout>
