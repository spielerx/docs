...
- Variable performance - Can have downtimw if fizz node is stopped by the user
...
- Real-time log streaming - your can follow the logs as they are generated
...
3. **Deployment Not Available**
   ...
   - You can check the logs for more details using flag `--follow` to see if there are any errors in the logs if the deployment is not available
