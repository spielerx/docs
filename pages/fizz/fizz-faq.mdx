...
2. **Select a New Provider**:
   ...
   - The list is sorted based on proximity, so it's recommended to select a provider nearest to you for optimal performance.
...
2. **Follow the Setup Guide**: 
   - Execute the steps in the setup guide, similar to your initial setup process.

3. **Check Your Status**: 
   - Verify your node's status to ensure everything is running smoothly.

By following these steps, you should be able to switch to a new provider and get your Fizz Node back up and running quickly. If you encounter any issues during this process, don't hesitate to reach out to our Discord community for assistance.
...
3. **Identify your GPU model**:
   Along with the command output, we need to know your GPU model name.

4. **Report the information**:
   Share both the command output and your GPU model name with our team. You can do this through our Discord channel.
...
Important Notes:
- Simply changing the script won't update your Fizz Node resources. You must update them through the dashboard first.
- If you've updated your script prior to this release, make sure to follow this "Update Node Resources" guide and run your node script again with the updated resources.

After completing these steps, your Fizz Node will be running with the new resource configuration. This update ensures that your node is correctly registered and can fully participate in the Fizz network.
